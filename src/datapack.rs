...
pub fn generate_abstract_template(path: &Utf8Path) -> Result<()> {
    ...
    let mut archive = ZipArchive::new(Cursor::new(zip_template)).context(Exception::Panic {
        reason: "Could not gather the data for the template",
    })?;

    if let Err(error) = archive.extract(path) {
        return Err(anyhow!(Exception::ZipError(
            format!("the path {}", path).to_string(),
            error
        )));
    }
